...
data = np.load('datatest1119.npy') 
labels = np.load('labelstest1119.npy')
print(data[0])
...
fit_result = model.fit(trainX, trainY, validation_data=(testX, testY), epochs=EPOCH, batch_size=3) #Для остановки обучения при возрастании ошибки: callbacks = k.callbacks.EarlyStopping(monitor='val_accuracy', patience=3)



#Построение графика
N = np.arange(0, fit_result.params['epochs'])#эпохи
...
plt.legend()
plt.savefig('thirdmodel') #TODO изменить имя файла

model.save('thirdmodel')#Сохранение модели