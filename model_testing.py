...
imgs = np.load('./img3/fortest/imgtest1119.npy') #Загрузка массива изображений


data = np.load('./img3/fortest/datatest1119.npy') #Загрузка массива
labels = np.load('labelstest1119.npy') #Загрузка массива меток
model = k.models.load_model('./thirdmodel') #Загрузка модели
...
for el in predictions:
    ...
    outimg = cv2.putText(imgs[j], text, (0,230), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
    # viewImage(imgs[j],'Images')
    cv2.imwrite(str(os.getcwd())+'/results/testwithoutbg/'+str(j)+'.jpg', imgs[j])
    # cv2.waitKey(0)
    j+=1
